  ...
  return (
    <section id="servicos" className="py-16 md:py-20 bg-[#09101d] relative">
      <div className="container mx-auto px-6 lg:px-8 max-w-7xl">
        ...
        {/* Cabeçalho animado */}
        <div ref={headerRef} className={`text-center mb-12 md:mb-16 anim-fade-up ${headerVisible ? 'visible' : ''}`}>
        ...
        {/* Grid com cards em cascata */}
        <div ref={gridRef} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
          {services.map((service, index) => {
            const IconComponent = service.icon;
            const isHighlighted = index < 3;
            const delayClass = `anim-delay-${Math.min(index, 7)}`;

            return (
              <article
                key={index}
                className={`group relative rounded-2xl p-6 bg-[#0f172a] border border-slate-800 hover:border-[#00c4cc]/60 shadow-lg hover:shadow-[0_8px_30px_rgba(0,196,204,0.15)] transition-all duration-300 flex flex-col justify-between h-full anim-scale-up ${delayClass} ${gridVisible ? 'visible' : ''}`}
              >
                <div>
                  {/* Ícone */}
          ...
          __html: JSON.stringify({
            ...
            itemListElement: services.map((s, idx) => ({
              ...
              item: {
                ...
                provider: { '@type': 'Organization', name: 'WeBuildSites', url: 'https://webuildsites.com.br' },
